...
# --- __call__ reduction on a backend (test_bovy14_callMargXZ, torch-only) ------
# streamdf.__call__ reduces the frequency/angle offsets with numpy.sum(...,
# axis=0). callMarg feeds __call__ phase-space coords from the (migrated)
# coords.rect_to_cyl transforms; under torch those are tensors and raw
# numpy.sum(tensor, axis=0) raised "sum() got an unexpected keyword argument
# 'axis'" (torch's reduction takes dim=, not axis=) -- the crash behind
# test_bovy14_callMargXZ (a torch-only ledger entry; jax's .sum accepts axis, so
# jax never hit the reduction bug). Routing the sums through the data's namespace
# (xp.sum) fixes it. Exercised directly at __call__ with torch phase-space
# tensors (the reduction site). NB: the stored numpy constants
# (self._sigomatrixinv ...) are not yet coerced onto the backend, so the
# surrounding numpy.dot/log/fabs still round-trip tensors through numpy's
# __array__ (a deferred Track-F migration of the full __call__) -- hence the
# filtered deprecation. jax __call__ is blocked upstream by a separate in-place
# assignment in prepData4Call (a distinct, unledgered jax gap).
_CALL_R = numpy.array([1.5, 1.6, 1.55])
_CALL_VR = numpy.array([0.1, 0.05, 0.08])
_CALL_VT = numpy.array([1.1, 1.05, 1.08])
_CALL_Z = numpy.array([0.1, 0.12, 0.09])
_CALL_VZ = numpy.array([0.05, 0.03, 0.04])
_CALL_PHI = numpy.array([0.2, 0.25, 0.22])
...
@pytest.mark.parametrize("logp", [True, False])
def test_call_reduction_torch(sdf, logp):
    args = (_CALL_R, _CALL_VR, _CALL_VT, _CALL_Z, _CALL_VZ, _CALL_PHI)
    ref = numpy.asarray(sdf(*args, log=logp))
    targs = tuple(torch.tensor(a, dtype=torch.float64) for a in args)
    out = sdf(*targs, log=logp)
    assert is_backend_array(out), "torch __call__ should reduce on the backend"
    numpy.testing.assert_allclose(
        as_numpy(out), ref, rtol=1e-9, atol=1e-9, err_msg="__call__ torch reduction"
    )
