...
# --- Approach 2: Accelerated piece-by-piece integration ---
def earth_PREM_mass_sym_fast():
    dens_sym, r = earth_PREM_density_sym()
    R = sp.Symbol("R", positive=True)
    EARTH_RADIUS_KM = 6371.0
    ...
    for expr, rmin, rmax in pieces:
        upper = sp.Min(R, rmax)
        lower = rmin
        cond = sp.And(R > rmin, upper > lower)
        integrals.append(sp.Piecewise(
            (4 * sp.pi * sp.integrate(expr * r**2, (r, lower, upper)), cond),
            (0, True)
        ))

    return sp.simplify(sum(integrals))
...
if __name__ == "__main__":
    ...
    # --- PieceWise method timing ---
    t3 = time.time()
    M_pieceWise_expr = earth_PREM_mass_sym_fast()
    t4 = time.time()
    M_pieceWise_val = M_pieceWise_expr.subs({"R": R_val})
    t5 = time.time()

    print("\n=== Fast piecewise integration ===")
    print(f"Symbolic build time: {t4 - t3:.6f} s")
    print(f"Numeric eval time  : {t5 - t4:.6f} s")
    print(f"Mass({R_val} km)   : {M_pieceWise_val.evalf()} g (PREM units)")

    # print("\nExpressions are equal:", sp.simplify(M_direct_expr - M_pieceWise_expr) == 0)

    def eval_with_min(expr, R_val, R):
    ...
    value = eval_with_min(M_direct_expr, 6371, R)
