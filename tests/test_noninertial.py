...
def test_lsrframe_scalaromegaz():
    ...
    def check_orbit(method='odeint',tol=1e-9):
        ...
        op_xs= op.x(ts)
        op_ys= op.y(ts)
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) <tol, f'Integrating an orbit in the rotating LSR frame does not agree with the equivalent orbit in the intertial frame for integration method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in the rotating LSR frame does not agree with the equivalent orbit in the intertial frame for integration method {method}'
...
def test_lsrframe_vecomegaz():
    ...
    def check_orbit(method='odeint',tol=1e-9):
        ...
        op_xs= op.x(ts)
        op_ys= op.y(ts)
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in the rotating LSR frame does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in the rotating LSR frame does not agree with the equivalent orbit in the intertial frame for method {method}'
    check_orbit(method='odeint',tol=1e-6)
    check_orbit(method='dop853_c',tol=1e-9)
    return None
...
def test_accellsrframe_scalaromegaz():
    ...
    def check_orbit(method='odeint',tol=1e-9):
        ...
        op_xs= op.x(ts)
        op_ys= op.y(ts)
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in the acceleratingly-rotating LSR frame does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in the acceleratingly-rotating LSR frame does not agree with the equivalent orbit in the intertial frame for method {method}'
    check_orbit(method='odeint',tol=1e-6)
    check_orbit(method='dop853_c',tol=1e-9)
    return None
...
def test_accellsrframe_funcomegaz():
    ...
    omega= lp.omegac(1.)
    omegadot= 0.02
    omega_func= lambda t: lp.omegac(1.)+0.02*t
...
def test_accellsrframe_vecfuncomegaz():
    ...
    omega= lp.omegac(1.)
    omegadot= 0.02
    omega_func= [lambda t: 0., lambda t: 0., lambda t: lp.omegac(1.)+0.02*t]
    omegadot_func= [lambda t: 0., lambda t: 0., lambda t: 0.02]
    diskpot= lp
    framepot= potential.NonInertialFrameForce(Omega=omega_func,Omegadot=omegadot_func)
    diskframepot= lp+framepot
    # Now integrate the orbit of the Sun in both the inertial and the lsr frame
...
def test_arbitraryaxisrotation_nullpotential():
    ...
    def check_orbit(zvec=[1.,0.,1.],omega=1.3,method='odeint',tol=1e-9):
        ...
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_zs-op_zs)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vRs-op_vRs)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vTs-op_vTs)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vzs-op_vzs)) < tol, f'Integrating an orbit in a rotating frame around an arbitrary axis does not agree with the equivalent orbit in the intertial frame for method {method}'
...
def test_arbitraryaxisrotation_omegafunc_nullpotential():
    ...
    # object should remain static
    np= potential.NullPotential()
    def check_orbit(zvec=[1.,0.,1.],omega=1.3,omegadot=0.1,omegadotdot=0.01,
                    method='odeint',tol=1e-9):
        # Set up the rotating frame's rotation matrix
        rotpot= potential.RotateAndTiltWrapperPotential(pot=np,zvec=zvec)
        rot= rotpot._rot
        # Now integrate an orbit in the inertial frame
        ...
        Omega= numpy.array(derive_noninert_omega(omega,rot=rot))
        Omegadot= numpy.array(derive_noninert_omega(omega,rot=rot))*omegadot/omega
        Omegadotdot= numpy.array(derive_noninert_omega(omega,rot=rot))*omegadotdot/omega
        op.integrate(ts,RotatingPotentialWrapperPotential(pot=np,rot=rot,omega=omega,
                                                          omegadot=omegadot,
                                                          omegadotdot=omegadotdot)
            +potential.NonInertialFrameForce(\
                Omega=[lambda t: Omega[0]+Omegadot[0]*t+Omegadotdot[0]*t**2./2.,
                       lambda t: Omega[1]+Omegadot[1]*t+Omegadotdot[1]*t**2./2.,
                       lambda t: Omega[2]+Omegadot[2]*t+Omegadotdot[2]*t**2./2.],
                Omegadot=[lambda t: Omegadot[0]+Omegadotdot[0]*t,
        ...
        for ii,t in enumerate(ts):
            xyz= rotate_and_omega(op.x(t),op.y(t),phi=op.z(t),t=t,
                                  rot=rot,omega=omega,omegadot=omegadot,omegadotdot=omegadotdot,
                                  rect=True)
            op_xs.append(xyz[0])
            op_ys.append(xyz[1])
            op_zs.append(xyz[2])
...
def test_arbitraryaxisrotation_omegafunc():
    ...
    dp= potential.DehnenBarPotential(omegab=1.8,rb=0.5,Af=0.03)
    diskpot= lp+dp
    def check_orbit(zvec=[1.,0.,1.],omega=1.3,omegadot=0.03,omegadotdot=0.01,
                    method='odeint',tol=1e-9):
        # Set up the rotating frame's rotation matrix
        rotpot= potential.RotateAndTiltWrapperPotential(pot=diskpot,zvec=zvec)
        rot= rotpot._rot
...
def test_linacc_constantacc_z():
    ...
    def check_orbit(method='odeint',tol=1e-9):
        ...
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in a linearly-accelerating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in a linearly-accelerating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_zs-op_zs)) < tol, f'Integrating an orbit in a linearly-accelerating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
...
def test_linacc_constantacc_xyz():
    ...
    diskframepot= AcceleratingPotentialWrapperPotential(pot=diskpot,x0=inta)\
                    +framepot
    def check_orbit(method='odeint',tol=1e-9):
        o= Orbit()
        o.turn_physical_off()
...
def test_linacc_changingacc_z():
    ...
    framepot= potential.NonInertialFrameForce(a0=[lambda t: 0.,lambda t: 0.,az])
    diskframepot= AcceleratingPotentialWrapperPotential(pot=diskpot,
                                                        x0=[lambda t: 0.,
                                                            lambda t: 0.,
                                                            intaz])\
...
def test_linacc_changingacc_xyz_accellsrframe_scalaromegaz():
    ...
    dp= potential.DehnenBarPotential(omegab=1.8,rb=0.5,Af=0.03)
    diskpot= lp+dp
    x0= [lambda t: -0.03*t**2./2.-0.03*t**3./6./20.,
         lambda t: 0.04*t**2./2.+0.08*t**3./6./20.,
         lambda t: 0.02*t**2./2.+0.03*t**3./6./20.]
    v0= [lambda t: -0.03*t-0.03*t**2./2./20.,
         lambda t: 0.04*t+0.08*t**2./2./20.,
         lambda t: 0.02*t+0.03*t**2./2./20.]
    a0= [lambda t: -0.03-0.03*t/20., \
         lambda t: 0.04+0.08*t/20., \
         lambda t: 0.02+0.03*t/20.]
    omega= lp.omegac(1.)
    omegadot= 0.02
    framepot= potential.NonInertialFrameForce(x0=x0,v0=v0,a0=a0,
                                              Omega=omega,
                                              Omegadot=omegadot)
    diskframepot= AcceleratingPotentialWrapperPotential(pot=diskpot,
                                                        x0=x0,
                                                        omegaz=omega,omegazdot=omegadot)\
                    +framepot
    def check_orbit(method='odeint',tol=1e-9):
        o= Orbit()
        o.turn_physical_off()
        ...
        phip+= omega*ts+omegadot*ts**2./2.
        op_xs, op_ys,_= coords.cyl_to_rect(Rp,phip,op_zs)
        op_vxs= op.vx(ts)+v0[0](ts)
        op_vys= op.vy(ts)+v0[1](ts)
        op_vzs= op.vz(ts)+v0[2](ts)
        vRp,vTp,_= coords.rect_to_cyl_vec(op_vxs,op_vys,op_vzs,
                                          op.x(ts)+x0[0](ts),op.y(ts)+x0[1](ts),op.z(ts)+x0[2](ts))
        vTp+= omega*Rp+omegadot*ts*Rp
        op_vxs, op_vys,_= coords.cyl_to_rect_vec(vRp,vTp,op_vzs,phi=phip)
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_zs-op_zs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vxs-op_vxs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vys-op_vys)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_vzs-op_vzs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
    check_orbit(method='odeint',tol=1e-5)
    check_orbit(method='dop853',tol=1e-9)
    check_orbit(method='dop853_c',tol=1e-5) # Lower tol, because diff integrators for inertial and non-inertial, bc wrapper not implemented in C
    return None
...
def test_linacc_changingacc_xyz_accellsrframe_scalarfuncomegaz():
    ...
    omegadot= 0.1
    omegadotdot= 0.01
    omega_func= lambda t: omega+omegadot*t+omegadotdot*t**2./2.
    omegadot_func= lambda t: omegadot+omegadotdot*t
    framepot= potential.NonInertialFrameForce(x0=x0,v0=v0,a0=a0,
                                              Omega=omega_func,
                                              Omegadot=omegadot_func)
    diskframepot= AcceleratingPotentialWrapperPotential(pot=diskpot,
                                                        x0=x0,
                                                        omegaz=omega,omegazdot=omegadot,
    ...
    def check_orbit(method='odeint',tol=1e-9):
        ...
        op_zs= op.z(ts)+x0[2](ts)
        Rp,phip,_= coords.rect_to_cyl(op_xs,op_ys,op_zs)
        phip+= omega*ts+omegadot*ts**2./2.+omegadotdot*ts**3./6.
        op_xs, op_ys,_= coords.cyl_to_rect(Rp,phip,op_zs)
        op_vxs= op.vx(ts)+v0[0](ts)
        op_vys= op.vy(ts)+v0[1](ts)
        op_vzs= op.vz(ts)+v0[2](ts)
        vRp,vTp,_= coords.rect_to_cyl_vec(op_vxs,op_vys,op_vzs,
                                          op.x(ts)+x0[0](ts),op.y(ts)+x0[1](ts),op.z(ts)+x0[2](ts))
        vTp+= omega*Rp+omegadot*ts*Rp+omegadotdot*ts**2./2.*Rp
        op_vxs, op_vys,_= coords.cyl_to_rect_vec(vRp,vTp,op_vzs,phi=phip)
        assert numpy.amax(numpy.fabs(o_xs-op_xs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_ys-op_ys)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
        assert numpy.amax(numpy.fabs(o_zs-op_zs)) < tol, f'Integrating an orbit in a linearly-accelerating, acceleratingly-rotating frame with constant acceleration does not agree with the equivalent orbit in the intertial frame for method {method}'
...
def test_linacc_changingacc_xyz_accellsrframe_funcomegaz():
    ...
    omegadot= 0.1
    omegadotdot= 0.01
    omega_func= [lambda t: 0., lambda t: 0., lambda t: omega+omegadot*t+omegadotdot*t**2./2.]
    omegadot_func= [lambda t: 0., lambda t: 0., lambda t: omegadot+omegadotdot*t]
    framepot= potential.NonInertialFrameForce(x0=x0,v0=v0,a0=a0,
                                              Omega=omega_func,
                                              Omegadot=omegadot_func)
    diskframepot= AcceleratingPotentialWrapperPotential(pot=diskpot,
...
def test_python_vs_c_arbitraryaxisrotation():
    ...
    def check_orbit(zvec=[1.,0.,1.],omega=1.3,
        ...
        assert numpy.amax(numpy.fabs(o.z(ts)-op.z(ts))) <tol, f'Integrating an orbit in a rotating frame in Python does not agree with integrating the same orbit in C; using methods {py_method} and {c_method}'
        ...
        assert numpy.amax(numpy.fabs(o.vz(ts)-op.vz(ts))) <tol, f'Integrating an orbit in a rotating frame in Python does not agree with integrating the same orbit in C; using methods {py_method} and {c_method}'
        return None
    check_orbit()
    return None
...
from galpy.potential.Potential import (_evaluatephitorques, _evaluateRforces,
                                       _evaluatezforces)
...
class AcceleratingPotentialWrapperPotential(parentWrapperPotential):
    def __init__(self,amp=1.,pot=None,
                 ...
                 omegaz=None,omegazdot=None,omegazdotdot=None,
                 ro=None,vo=None):
    ...
    def _force_xyz(self,R,z,phi=0.,t=0.):
        ...
        if not self._omegaz is None:
            ...
            if not self._omegazdot is None:
                rotphi+= self._omegazdot*t**2./2.
            if not self._omegazdotdot is None:
                rotphi+= self._omegazdotdot*t**3./6.
            return numpy.dot(numpy.array([[ numpy.cos(rotphi),numpy.sin(rotphi),0.],
                                          [-numpy.sin(rotphi),numpy.cos(rotphi),0.],
                                          [0.,0.,1.]]),
...
def rotate_and_omega(R,z,phi=0.,t=0.,rot=None,omega=None,omegadot=None,omegadotdot=None,
                     rect=False):
...
class RotatingPotentialWrapperPotential(parentWrapperPotential):
    def __init__(self,amp=1.,pot=None,rot=None,omega=None,omegadot=None,omegadotdot=None,
                 ro=None,vo=None):
    ...
    def _force_xyz(self,R,z,phi=0.,t=0.):
        ...
        inv_rot= numpy.array([\
           list(rotate_and_omega(1.,0.,phi=0.,t=t,rot=self._rot,omega=self._omega,omegadot=self._omegadot,omegadotdot=self._omegadotdot,rect=True)),
           list(rotate_and_omega(0.,1.,phi=0.,t=t,rot=self._rot,omega=self._omega,omegadot=self._omegadot,omegadotdot=self._omegadotdot,rect=True)),
           list(rotate_and_omega(0.,0.,phi=1.,t=t,rot=self._rot,omega=self._omega,omegadot=self._omegadot,omegadotdot=self._omegadotdot,rect=True))])
