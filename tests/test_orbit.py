...
def test_energy_jacobi_conservation():
    ...
    for p in pots:
        ...
        if p in list(tol.keys()):
            ...
            ttol = tol["default"]
        if p in list(jactol.keys()):
            tjactol = jactol[p]
        else:
            tjactol = jactol["default"]
        try:
        ...
        for integrator in integrators:
            if integrator == "dopr54_c" and (
                "Spiral" in p or "Lopsided" in p or "Dehnen" in p or "Cosmphi" in p
            ):
                ttimes = growtimes
            elif (
                integrator == "dopr54_c"
            ...
            if "MovingObject" in p:
                if _QUICKTEST and not (
                    ("NFW" in p and not tp.isNonAxi and "SCF" not in p)
            ...
            if "AnyAxisymmetricRazorThinDiskPotential" in p:
                break
            # Now do axisymmetric
            if not tp.isNonAxi:
                o = setup_orbit_energy(tp, axi=True, henon="Henon" in p)
                if isinstance(tp, testMWPotential) or isinstance(
                    tp, testplanarMWPotential
                ):
                    o.integrate(ttimes, tp._potlist, method=integrator)
                else:
                    o.integrate(ttimes, tp, method=integrator)
            ...
            if p == "mockRotatedAndTiltedMWP14WrapperPotential":
                break
                #            print integrator
    ...
    for p in pots:
        ...
        for integrator in integrators:
            if integrator == "dopr54_c" and (
                ...
                and not "MovingObject" in p
                and not p == "FerrersPotential"
                and not p == "AnyAxisymmetricRazorThinDiskPotential"
            ):
                ttimes = times
            else:
                ttimes = fasttimes
            ...
            if _QUICKTEST and not (
                p == "NFWPotential" or ("Burkert" in p and not tp.hasC)
            ):
                break
    return None
    ...
    for p in pots:
        ...
        for integrator in integrators:
            ...
            assert numpy.fabs(tjac - 1.0) < 10.0**ttol, (
                "Liouville theorem jacobian differs from one by {:g} for {} and integrator {}".format(
                    numpy.fabs(tjac - 1.0), p, integrator
                )
            )
...
def test_analytic_ecc_rperi_rap():
    ...
    )  # fails currently, because delta esimation gives a NaN due to a 0/0; delta should just be zero, but don't want to special-case
    # No C and therefore annoying
    rmpots.append("AnyAxisymmetricRazorThinDiskPotential")
    if False:  # _GHACTIONS:
        rmpots.append("DoubleExponentialDiskPotential")
...
def test_fixedstepsize():
    if WIN32:
        return None  # skip on appveyor, because fails for reason that I can't figure out (runtimes[0] == 0.) and not that important
    from galpy.potential import LogarithmicHaloPotential
    import time
...
def test_fixedstepsize_dxdv():
    if WIN32:
        return None  # skip on appveyor, because test_fixedstepsize fails for reason that I can't figure out (runtimes[0] == 0.) and not that important
    from galpy.potential import LogarithmicHaloPotential
    import time
...
def test_full_plotting():
    ...
    o.plot(
        d1="R*cos(phi-{:f}*t)".format(o.Op(quantity=False) - o.Or(quantity=False) / 2),
        d2="R*sin(phi-{:f}*t)".format(o.Op(quantity=False) - o.Or(quantity=False) / 2),
    )
    with pytest.raises(TypeError) as excinfo:
    ...
    for obj in named_data:
        ...
        for attr in named_data[obj]:
            ...
            if attr == "ro" or attr == "vo" or attr == "zo" or attr == "solarmotion":
                assert numpy.all(
                    numpy.isclose(
                        getattr(o, "_{:s}".format(attr)), named_data[obj][attr]
                    )
                )
            elif attr == "distance":
                ...
                assert numpy.isclose(
                    getattr(o, "{:s}".format(attr))(), named_data[obj][attr]
                )
    return None
...
def test_noDeprecationWarning_timeInCall():
    ...
    with pytest.warns(None) as record:
        ...
        assert not raisedWarning, (
            "Orbit evaluation with array times raises the DeprecationWarning 'elementwise == comparison failed; this will raise an error in the future.'"
        )
    # Also when using time units and evaluating without
    ts = numpy.linspace(0.0, 0.1, 1001) * units.Gyr
    orb.integrate(ts, MWPotential)
    with pytest.warns(None) as record:
...
def check_radecetc_roWarning(o, funcName):
    ...
    for rec in record:
        ...
        raisedWarning += (
            str(rec.message.args[0])
            == "Method {}(.) requires ro to be given at Orbit initialization or at method evaluation; using default ro which is {:f} kpc".format(
                funcName, 8.0
            )
        )
    assert raisedWarning, (
...
def check_radecetc_voWarning(o, funcName):
    ...
    for rec in record:
        ...
        raisedWarning += (
            str(rec.message.args[0])
            == "Method {}(.) requires vo to be given at Orbit initialization or at method evaluation; using default vo which is {:f} km/s".format(
                funcName, 220.0
            )
        )
    assert raisedWarning, (
...
def check_integrate_t_asQuantity_warning(o, funcName):
    ...
    assert raisedWarning, (
        "Orbit method %s wit unitless time after integrating with unitful time should have thrown a warning, but didn't"
        % funcName
    )
    return None
