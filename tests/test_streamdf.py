...
def test_bovy14_useTM_poterror():
    if WIN32:
        return None  # skip on appveyor, because no TM
...
def check_track_prog_diff(sdf, d1, d2, tol, phys=False):
    ...
    assert maxdevZ < tol, (
        "Stream track deviates more from progenitor track in {} vs. {} than expected; max. deviation = {:f}".format(
            d2, d1, maxdevZ
        )
    )
    return None
...
def check_track_spread(sdf, d1, d2, tol1, tol2, phys=False, interp=True):
    ...
    assert numpy.amax(addx) < tol1, (
        "Stream track spread is larger in {} than expected; max. deviation = {:f}".format(
            d1, numpy.amax(addx)
        )
    )
