...
def test_scf_compute_spherical_hernquist():
    ...
    assert numpy.fabs(Acos[0, 0, 0] - 1.0) < EPS, (
        "Acos(n=0,l=0,m=0) = 1 fails. Found to be Acos(n=0,l=0,m=0) = {}".format(
            Acos[0, 0, 0]
        )
    )
...
def test_scf_compute_spherical_zeeuw():
    ...
    assert numpy.fabs(Acos[0, 0, 0] - 2 * 3.0 / 4) < EPS, (
        "Acos(n=0,l=0,m=0) = 3/2 fails. Found to be Acos(n=0,l=0,m=0) = {}".format(
            Acos[0, 0, 0]
        )
    )
    assert numpy.fabs(Acos[1, 0, 0] - 2 * 1.0 / 12) < EPS, (
        "Acos(n=1,l=0,m=0) = 1/6 fails. Found to be Acos(n=0,l=0,m=0) = {}".format(
            Acos[0, 0, 0]
        )
    )
    ...
    for n in range(shape[0]):
        for l in range(shape[1]):
            assert (
                ...
                "Acos(n={},l={},0) = {}, whereas it was analytically calculated to be {}".format(
                    n, l, numerically_calculated[n, l], analytically_calculated[n, l]
                )
            )
...
def axi_reducesto_spherical(Aspherical, Aaxi, potentialName):
    ...
    assert numpy.all(numpy.fabs(Acos_s[:n, 0, 0] - Acos_a[:n, 0, 0]) < EPS), (
        "The axi symmetric Acos(n,l=0,m=0) does not reduce to the spherical Acos(n,l=0,m=0) for {}".format(
            potentialName
        )
    )
...
def reducesto_spherical(Aspherical, A, potentialName):
    ...
    assert numpy.all(numpy.fabs(Acos_s[:n, 0, 0] - Acos[:n, 0, 0]) < EPS), (
        "Acos(n,l=0,m=0) as generated by scf_compute_coeffs does not reduce to the spherical Acos(n,l=0,m=0) for {}".format(
            potentialName
        )
    )
