...
def test_potential_function_inputAsQuantity():
    ...
    assert (
        numpy.fabs(
            potential.rE(
                pot[0], -1.14 * vo**2 * units.km**2 / units.s**2, use_physical=False
            )
            - potential.rE(potu, -1.14)
        )
        < 10.0**-8.0
...
def test_potential_ampunits_altunits():
    ...
    # DoubleExponentialDiskPotential
    pot = potential.DoubleExponentialDiskPotential(
        amp=0.1 * units.Msun / units.pc**3.0 * constants.G, hr=2.0, hz=0.2, ro=ro, vo=vo
    )
    # density at zero should be amp
    assert (
        numpy.fabs(
    ...
    # MN3ExponentialDiskPotential
    pot = potential.MN3ExponentialDiskPotential(
        amp=0.1 * units.Msun * constants.G / units.pc**3.0, hr=2.0, hz=0.2, ro=ro, vo=vo
    )
    # density at hr should be
    assert (
        numpy.fabs(
...
def test_potential_ampunits_wrongunits():
    ...
    with pytest.raises(units.UnitConversionError) as excinfo:
        potential.PowerSphericalPotential(
            amp=10.0**10.0 * units.Msun / units.pc**3, r1=1.0, alpha=2.0, ro=ro, vo=vo
        )
    # PowerSphericalPotentialwCutoff
    with pytest.raises(units.UnitConversionError) as excinfo:
        potential.PowerSphericalPotentialwCutoff(
            amp=0.1 * units.Msun / units.pc**2, r1=1.0, alpha=2.0, rc=2.0, ro=ro, vo=vo
        )
    # PseudoIsothermalPotential
    with pytest.raises(units.UnitConversionError) as excinfo:
...
def test_potential_paramunits():
    ...
    # PerfectEllipsoidPotential
    pot = potential.PerfectEllipsoidPotential(
        amp=4.0 * 10.0**10.0 * units.Msun, a=5.0 * units.kpc, ro=ro, vo=vo, b=1.3, c=0.4
    )
    pot_nounits = potential.PerfectEllipsoidPotential(
...
def test_potential_paramunits_2d():
    ...
    pot_nounits = potential.CosmphiDiskPotential(
        amp=1.0, m=3, cp=1000.0 / vo**2.0, sp=300.0 / vo**2.0, r1=8.0 / ro, ro=ro, vo=vo
    )
    # Check potential
    assert (
        numpy.fabs(
...
def test_streamgapdf_inputAsQuantity():
    ...
    assert (
        ...
        < _NUMPY_1_22 * 1e-4 + (1 - _NUMPY_1_22) * 1e-8
    ), (
...
def test_streamgapdf_sample():
    ...
    assert (
        numpy.fabs(RvR[3].to(units.kpc).value / sdf_sanders15._ro - RvRnou[3])
    ...
    assert (
        numpy.fabs(RvR[5].to(units.rad).value - RvRnou[5])
        < _NUMPY_1_22 * 1e-6 + (1 - _NUMPY_1_22) * 1e-8
    ), "streamgapdf sample RvR does not return a correct Quantity"
    # RvR,dt
    numpy.random.seed(1)
    ...
    assert (
        numpy.fabs(RvRdt[5].to(units.rad).value - RvRdtnou[5])
        < _NUMPY_1_22 * 1e-6 + (1 - _NUMPY_1_22) * 1e-8
    ), "streamgapdf sample RvRdt does not return a correct Quantity"
    assert (
        numpy.fabs(
    ...
    assert (
        numpy.fabs(xy[0].to(units.kpc).value / sdf_sanders15._ro - xynou[0])
        < _NUMPY_1_22 * 1e-6 + (1 - _NUMPY_1_22) * 1e-8
    ), "streamgapdf sample xy does not return a correct Quantity"
    assert (
        numpy.fabs(xy[1].to(units.kpc).value / sdf_sanders15._ro - xynou[1])
        < _NUMPY_1_22 * 1e-6 + (1 - _NUMPY_1_22) * 1e-8
    ), "streamgapdf sample xy does not return a correct Quantity"
    assert (
        numpy.fabs(xy[2].to(units.kpc).value / sdf_sanders15._ro - xynou[2])
        < _NUMPY_1_22 * 1e-6 + (1 - _NUMPY_1_22) * 1e-8
    ), "streamgapdf sample xy does not return a correct Quantity"
    assert (
