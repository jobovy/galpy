...
@pytest.mark.filterwarnings("ignore:.*requires_grad.*:UserWarning")
def test_scalar_grad_through_a_wrapper_potential():
    # A wrapper keeps the differentiated parameter one level in, on the wrapped
    # potential, so _pot_grad_namespace has to recurse to find it. Without that
    # recursion rl falls to the scipy branch and raises on the tracer.
    if not _HAS_JAX:  # pragma: no cover - jax not installed
        pytest.skip("jax not installed")
    from galpy.potential import DehnenSmoothWrapperPotential
    from galpy.potential import MiyamotoNagaiPotential as _MN

    def rl_of_a(a):
        wrapped = DehnenSmoothWrapperPotential(
            pot=_MN(amp=1.0, a=a, b=0.3), tform=-1.0, tsteady=0.5
        )
        return rl_fn(wrapped, 0.8, use_physical=False)

    h = 1e-6 * _PGRAD_A0
    fd = (float(rl_of_a(_PGRAD_A0 + h)) - float(rl_of_a(_PGRAD_A0 - h))) / (2.0 * h)
    assert abs(fd) > 1e-6, "finite difference is ~0: the test would prove nothing"
    ad = float(jax.grad(rl_of_a)(jnp.asarray(_PGRAD_A0)))
