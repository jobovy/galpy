...
def test_impulse_deltav_plummerstream():
    ...
    assert numpy.all(
        ...
        * (numpy.fabs(kick) >= 10**-4.0)
        ...
        "Short stream impulse kick calculation does not agree with Plummer calculation by rel: {:g}, abs: {:g}".format(
            numpy.amax(
                numpy.fabs((kick - stream_kick) / kick)[numpy.fabs(kick) >= 10**-4.0]
            ),
            numpy.amax(numpy.fabs(kick - stream_kick)[numpy.fabs(kick) < 10**-3.0]),
        )
    )
