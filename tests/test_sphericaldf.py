...
def test_constantbetadf_against_hernquist():
    if WIN32:
        return None  # skip on appveyor, because no JAX
    pot = potential.HernquistPotential(amp=2.3, a=1.3)
...
def test_isotropic_hernquist_nopot():
    ...
    assert str(excinfo.value) == "pot= must be potential.HernquistPotential", (
...
def test_isotropic_hernquist_unitsofsamples():
    ...
    dfh = isotropicHernquistdf(pot=pot)
    samp = dfh.sample(n=100)
    assert conversion.get_physical(samp, include_set=True)["roSet"], (
        "Orbit samples from spherical DF with units on do not have units on"
    )
    assert conversion.get_physical(samp, include_set=True)["voSet"], (
        "Orbit samples from spherical DF with units on do not have units on"
    )
    assert (
    ...
    dfh = isotropicHernquistdf(pot=pot)
    samp = dfh.sample(n=100)
    assert not conversion.get_physical(samp, include_set=True)["roSet"], (
        "Orbit samples from spherical DF with units off do not have units off"
    )
    assert not conversion.get_physical(samp, include_set=True)["voSet"], (
        "Orbit samples from spherical DF with units off do not have units off"
    )
    return None
...
def check_spherical_symmetry(samp, l, m, tol):
    ...
    assert (
        ...
        "Sample does not appear to be spherically symmetric, fails spherical harmonics test for (l,m) = ({},{})".format(
            l, m
        )
    )
    return None
...
def check_azimuthal_symmetry(samp, m, tol):
    ...
    assert numpy.fabs(numpy.sum(numpy.cos(m * phis)) / samp.size - (m == 0)) < tol, (
        "Sample does not appear to be azimuthally symmetric, fails Fourier test for m = {}".format(
            m
        )
    )
    return None
...
def check_beta(samp, pot, tol, beta=0.0, rmin=None, rmax=None, bins=31):
    ...
    mvt2, _ = numpy.histogram(
        logrs, range=[numpy.log(rmin), numpy.log(rmax)], bins=bins, weights=vthetas**2.0
    )
    ...
    if not callable(beta):
        ...
        beta_func = beta
    assert numpy.all(numpy.fabs(samp_beta - beta_func(brs)) < tol), (
        "beta(r) from samples does not agree with the expected value for beta = {}".format(
            beta
        )
    )
    return None
...
def check_dens_directint(dfi, pot, tol, dens, rmin=None, rmax=None, bins=31):
    ...
    assert numpy.all(numpy.fabs(intdens / expdens - 1.0) < tol), (
