...
# --- obs=[...] accessors under a forced backend (issue #322 helio/UVW) ----------
# helioX/Y and U/V/W with a list ``obs`` subtract the observer's angle/position
# from the (numpy-integrated) trajectory: ``thiso - arctan2(obs[1], obs[0])``.
# Under a forced backend the observer components (obs.x() ...) are backend
# tensors while ``thiso`` is numpy, so the mix raised ``numpy.ndarray - Tensor``
# (test_orbit_obs/obsvel_Orbits_issue322). _coerce_obs_thiso brings both onto the
# one namespace. NB: the downstream coords.galcenrect_to_XYZ rotation is still a
# numpy (einsum/dot) path, so the *result* comes back as numpy and the tensors
# round-trip through numpy's __array__ (a deferred coords migration) -- hence the
# filtered deprecation and the value-parity (not is_backend_array) assertion.
...
@pytest.mark.parametrize("backend_name", _FORCE_BACKENDS)
def test_obs_list_accessors_forced_backend(backend_name):
    times = numpy.linspace(0.0, 2.0, 3)
    # planar orbit + planar observer orbit, both integrated on numpy
    o = Orbit([0.9, 0.1, 1.2, numpy.pi / 2.0], ro=1.0)
    obs = Orbit([1.0, 0.5, 1.3, numpy.pi / 2.0], ro=1.0)
    o.integrate(times, _POT)
    obs.integrate(times, _POT)
    ii = 2
    t = times[ii]
    # numpy reference: obs components are numpy
    obs_pos = [obs.x(t), obs.y(t), 0.0]
    obs_full = [obs.x(t), obs.y(t), 0.0, obs.vx(t), obs.vy(t), 0.0]
    ref = {
        "helioX": float(o.helioX(t, obs=obs_pos, ro=1.0)),
        "helioY": float(o.helioY(t, obs=obs_pos, ro=1.0)),
        "U": float(o.U(t, obs=obs_full, ro=1.0)),
        "V": float(o.V(t, obs=obs_full, ro=1.0)),
    }
    with use(backend_name, force=True):
        # obs components computed on the forced backend are backend tensors,
        # exactly as the caller assembles them in the ledgered tests.
        b_pos = [obs.x(t), obs.y(t), 0.0]
        b_full = [obs.x(t), obs.y(t), 0.0, obs.vx(t), obs.vy(t), 0.0]
        assert is_backend_array(b_pos[0]), "obs.x() should be on the forced backend"
        got = {
            "helioX": float(as_numpy(o.helioX(t, obs=b_pos, ro=1.0))),
            "helioY": float(as_numpy(o.helioY(t, obs=b_pos, ro=1.0))),
            "U": float(as_numpy(o.U(t, obs=b_full, ro=1.0))),
            "V": float(as_numpy(o.V(t, obs=b_full, ro=1.0))),
        }
    for k in ref:
        numpy.testing.assert_allclose(
            got[k], ref[k], rtol=1e-10, atol=1e-12, err_msg=f"{k} forced-{backend_name}"
        )
