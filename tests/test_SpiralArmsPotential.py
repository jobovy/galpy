...
class TestSpiralArmsPotential(unittest.TestCase):
    ...
    def test_Rforce(self):
        ...
        assert_allclose(
            pot.Rforce(R, z, pi / 2.2, t),
            -deriv(lambda x: pot(x, z, pi / 2.2, t), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, pi, t),
            -deriv(lambda x: pot(x, z, pi, t), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 3.7 * pi / 2, t),
            -deriv(lambda x: pot(x, z, 3.7 * pi / 2, t), R, dx=dx),
            rtol=rtol,
        )
        R, z, t = 1, -0.7, 3
        assert_allclose(
        ...
        assert_allclose(
            pot.Rforce(R, z, pi / 2, t),
            -deriv(lambda x: pot(x, z, pi / 2, t), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, pi, t),
            -deriv(lambda x: pot(x, z, pi, 0), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 3.3 * pi / 2, t),
            -deriv(lambda x: pot(x, z, 3.3 * pi / 2, t), R, dx=dx),
            rtol=rtol,
        )
        R, z = 3.14, 0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.Rforce(R, z, pi / 2),
            -deriv(lambda x: pot(x, z, pi / 2), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, pi), -deriv(lambda x: pot(x, z, pi), R, dx=dx), rtol=rtol
        )
        assert_allclose(
            pot.Rforce(R, z, 3 * pi / 2),
            -deriv(lambda x: pot(x, z, 3 * pi / 2), R, dx=dx),
            rtol=rtol,
        )
        ...
        assert_allclose(
            pot.Rforce(R, z, 3 * pi / 2, t),
            -deriv(lambda x: pot(x, z, 3 * pi / 2, t), R, dx=dx),
            rtol=rtol,
        )
        R, z, t = 1, -0.7, 121
        assert_allclose(
        ...
        pot = spiral(
            N=10, r_ref=15, phi_ref=5, Cs=[8.0 / (3.0 * pi), 0.5, 8.0 / (15.0 * pi)]
        )
        assert_allclose(
            pot.Rforce(1.0, 0.0), -deriv(lambda x: pot(x, 0.0), 1.0, dx=dx), rtol=rtol
        )
        ...
        assert_allclose(
            pot.Rforce(R, z, pi / 2.1),
            -deriv(lambda x: pot(x, z, pi / 2.1), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 1.3 * pi),
            -deriv(lambda x: pot(x, z, 1.3 * pi), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 3 * pi / 2),
        ...
        assert_allclose(
            pot.Rforce(R, z, 0.9 * pi),
            -deriv(lambda x: pot(x, z, 0.9 * pi), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 3.3 * pi / 2),
            -deriv(lambda x: pot(x, z, 3.3 * pi / 2), R, dx=dx),
            rtol=rtol,
        )
        R, z = 3.14, 0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.Rforce(R, z, pi / 2.3),
            -deriv(lambda x: pot(x, z, pi / 2.3), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 1.1 * pi),
            -deriv(lambda x: pot(x, z, 1.1 * pi), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.Rforce(R, z, 3.5 * pi / 2),
            -deriv(lambda x: pot(x, z, 3.5 * pi / 2), R, dx=dx),
            rtol=rtol,
        )
    ...
    def test_zforce(self):
        ...
        assert_allclose(
            pot.zforce(R, z, pi / 2),
            -deriv(lambda x: pot(R, x, pi / 2), z, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.zforce(R, z, pi), -deriv(lambda x: pot(R, x, pi), z, dx=dx), rtol=rtol
        )
        assert_allclose(
            pot.zforce(R, z, 3 * pi / 2),
            -deriv(lambda x: pot(R, x, 3 * pi / 2), z, dx=dx),
            rtol=rtol,
        )
        R, z = 3.7, 0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.zforce(R, z, pi / 2, t),
            -deriv(lambda x: pot(R, x, pi / 2, t), z, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.zforce(R, z, pi, t),
            -deriv(lambda x: pot(R, x, pi, t), z, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.zforce(R, z, 3 * pi / 2, t),
            -deriv(lambda x: pot(R, x, 3 * pi / 2, t), z, dx=dx),
            rtol=rtol,
        )
        R, z = 3.7, 0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.zforce(R, z, 3 * pi / 2.1),
            -deriv(lambda x: pot(R, x, 3 * pi / 2.1), z, dx=dx),
            rtol=rtol,
        )
        R, z, t = 3.7, 0.7, -100
        assert_allclose(
        ...
        assert_allclose(
            pot.zforce(R, z, 3.4 * pi / 2, t),
            -deriv(lambda x: pot(R, x, 3.4 * pi / 2, t), z, dx=dx),
            rtol=rtol,
        )

        pot = spiral(
            N=5, r_ref=1.5, phi_ref=0.5, Cs=[8.0 / (3.0 * pi), 0.5, 8.0 / (15.0 * pi)]
        )
        # zforce is zero in the plane of the galaxy
        assert_allclose(0, pot.zforce(0.3, 0, 0), rtol=rtol)
    ...
    def test_phitorque(self):
        ...
        assert_allclose(
            pot.phitorque(R, z, pi / 2),
            -deriv(lambda x: pot(R, z, x), pi / 2, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phitorque(R, z, pi),
            -deriv(lambda x: pot(R, z, x), pi, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phitorque(R, z, 3 * pi / 2),
            -deriv(lambda x: pot(R, z, x), 3 * pi / 2, dx=dx),
            rtol=rtol,
        )
        R, z = 0.1, -0.3
        assert_allclose(
        ...
        assert_allclose(
            pot.phitorque(R, z, pi / 2.1),
            -deriv(lambda x: pot(R, z, x), pi / 2.1, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phitorque(R, z, pi),
        ...
        assert_allclose(
            pot.phitorque(R, z, pi / 2, t),
            -deriv(lambda x: pot(R, z, x, t), pi / 2, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phitorque(R, z, pi, t),
            -deriv(lambda x: pot(R, z, x, t), pi, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phitorque(R, z, 3 * pi / 2, t),
            -deriv(lambda x: pot(R, z, x, t), 3 * pi / 2, dx=dx),
            rtol=rtol,
        )
        R, z = 1, -0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.phitorque(R, z, 3.2 * pi / 2, t),
            -deriv(lambda x: pot(R, z, x, t), 3.2 * pi / 2, dx=dx),
            rtol=rtol,
        )
        ...
        assert_allclose(
            pot.phitorque(R, z, 3.2 * pi / 2),
            -deriv(lambda x: pot(R, z, x), 3.2 * pi / 2, dx=dx),
            rtol=rtol,
        )
        R, z, t = 1, -0.7, 123
        assert_allclose(
        ...
        pot = spiral(
            N=4, r_ref=1.5, phi_ref=5, Cs=[8.0 / (3.0 * pi), 0.5, 8.0 / (15.0 * pi)]
        )
        R, z = 0.3, 0
        assert_allclose(
            pot.phitorque(R, z, 0), -deriv(lambda x: pot(R, z, x), 0, dx=dx), rtol=rtol
        ...
        assert_allclose(
            pot.phitorque(R, z, 2 * pi),
            -deriv(lambda x: pot(R, z, x), 2 * pi, dx=dx),
            rtol=rtol,
        )
    ...
    def test_R2deriv(self):
        ...
        assert_allclose(
            pot.R2deriv(R, z, pi / 2),
            -deriv(lambda x: pot.Rforce(x, z, pi / 2), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.R2deriv(R, z, pi),
            -deriv(lambda x: pot.Rforce(x, z, pi), R, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.R2deriv(R, z, 3.1 * pi / 2),
            -deriv(lambda x: pot.Rforce(x, z, 3.1 * pi / 2), R, dx=dx),
            rtol=rtol,
        )
        R, z = 1, -0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.R2deriv(R, z, 2 * pi),
            -deriv(lambda x: pot.Rforce(x, z, 2 * pi), R, dx=dx),
            rtol=rtol,
        )
        R, z = 5, 0.9
        assert_allclose(
        ...
        assert_allclose(
            pot.R2deriv(R, z, 3 * pi / 2),
            -deriv(lambda x: pot.Rforce(x, z, 3 * pi / 2), R, dx=dx),
            rtol=rtol,
        )
    ...
    def test_z2deriv(self):
        ...
        assert_allclose(
            pot.z2deriv(R, z, pi / 2),
            -deriv(lambda x: pot.zforce(R, x, pi / 2), z, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.z2deriv(R, z, pi),
            -deriv(lambda x: pot.zforce(R, x, pi), z, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.z2deriv(R, z, 3 * pi / 2),
            -deriv(lambda x: pot.zforce(R, x, 3 * pi / 2), z, dx=dx),
            rtol=rtol,
        )
        R, z = 1, -0.3
        assert_allclose(
        ...
        pot = spiral(
            N=3, alpha=-0.3, r_ref=0.25, Cs=[8.0 / (3.0 * pi), 0.5, 8.0 / (15.0 * pi)]
        )
        R, z = 0.3, 0
        assert_allclose(
            pot.z2deriv(R, z, 0),
    ...
    def test_phi2deriv(self):
        ...
        assert_allclose(
            pot.phi2deriv(R, z, pi / 2.1),
            -deriv(lambda x: pot.phitorque(R, z, x), pi / 2.1, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phi2deriv(R, z, pi),
            -deriv(lambda x: pot.phitorque(R, z, x), pi, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phi2deriv(R, z, 3 * pi / 2.5),
            -deriv(lambda x: pot.phitorque(R, z, x), 3 * pi / 2.5, dx=dx),
            rtol=rtol,
        )
        R, z = 1, -0.3
        assert_allclose(
        ...
        assert_allclose(
            pot.phi2deriv(R, z, pi / 2),
            -deriv(lambda x: pot.phitorque(R, z, x), pi / 2, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phi2deriv(R, z, pi),
        ...
        assert_allclose(
            pot.phi2deriv(R, z, 3 * pi / 2),
            -deriv(lambda x: pot.phitorque(R, z, x), 3 * pi / 2, dx=dx),
            rtol=rtol,
        )
        R, z = 3.3, 0.7
        assert_allclose(
        ...
        assert_allclose(
            pot.phi2deriv(R, z, 3.3 * pi / 2),
            -deriv(lambda x: pot.phitorque(R, z, x), 3.3 * pi / 2, dx=dx),
            rtol=rtol,
        )
        R, z = 1, -0.3
        assert_allclose(
        ...
        assert_allclose(
            pot.phi2deriv(R, z, pi / 2.2),
            -deriv(lambda x: pot.phitorque(R, z, x), pi / 2.2, dx=dx),
            rtol=rtol,
        )
        assert_allclose(
            pot.phi2deriv(R, z, pi),
    ...
    def test_dens(self):
        ...
        assert_allclose(
            pot.dens(3, 2, pi, forcepoisson=False),
            pot.dens(3, 2, pi, forcepoisson=True),
            rtol=rtol,
        )
        assert_allclose(
            pot.dens(1, -1, -1, forcepoisson=False),
    ...
    def test_Rzderiv(self):
        ...
        R, z, phi, t = 0.777, 0.333, pi / 3, 0.0
        assert_allclose(
            pot.Rzderiv(R, z, phi, t),
            -deriv(lambda x: pot.Rforce(R, x, phi, t), z, dx=dx),
            rtol=rtol,
    ...
    def test_Rphideriv(self):
        ...
        pot = spiral(N=3, alpha=0.21, r_ref=0.5, phi_ref=pi, Cs=[2.0], omega=-3)
        R, z, phi, t = 1, 0, 0, 0
        assert_allclose(
            pot.Rphideriv(R, z, phi, t),
            -deriv(lambda x: pot.Rforce(R, z, x, t), phi, dx=dx),
        ...
        assert_allclose(
            ...
            [
                ...
                * (
                    float(phi)
                    - pot._phi_ref
                    - np.log(float(R) / pot._r_ref) / np.tan(pot._alpha)
                )
            ],
        )
