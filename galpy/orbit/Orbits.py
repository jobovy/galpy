...
class Orbit:
    ...
    def __init__(
        ...
        if self.phasedim() < 2 or self.phasedim() > 6:
            if len(self.vxvv) > 1 and len(self.vxvv) < 7:
                raise RuntimeError(
                    "Invalid phase-space dimension {:d} for {:d} objects; perhaps you meant to transpose the input?".format(
                        self.phasedim(), len(self.vxvv)
                    )
                )
            else:
                raise RuntimeError(
                    "Invalid phase-space dimension: phasedim = {:d}, but should be between 2 and 6".format(
                        self.phasedim()
                    )
                )
    ...
    def integrate(
        ...
        if _isDissipative(self._pot) and ("leapfrog" in method or "symplec" in method):
            if "_c" in method:
                method = "dopr54_c"
            else:
                method = "odeint"
    ...
    def integrate_dxdv(
        ...
        if method.lower() not in [
            ...
            if "leapfrog" in method.lower() or "symplec" in method.lower():
                raise ValueError(
                    "{:s} is not a valid `method for integrate_dxdv, because symplectic integrators cannot be used`".format(
                        method
                    )
                )
            else:
    ...
    def _parse_plot_quantity(self, quant, **kwargs):
        ...
        def _eval(q):
            ...
            if q == "t":
                ...
                return numpy.tile(self.time(self.t, **kwargs), (len(self.vxvv), 1))
            elif q == "Enorm":
        ...
        try:
            ...
            raise TypeError(
                'Parsing the expression {} failed, with error message:\n"{}"'.format(
                    quant, err
                )
            )
...
"""
        ...
        if json_filename is None:
            ...
            json_code = """Plotly.d3.json('{jfilename}',function(data){{""".format(
                jfilename=json_filename
            )
            close_json_code = "});"
        self.divid = "galpy-" + "".join(choice(ascii_lowercase) for i in range(24))
...
""".format(
            ...
            traces_cumul += """,trace{trace_num_1},trace{trace_num_2}""".format(
                trace_num_1=str(2 * ii + 1), trace_num_2=str(2 * ii + 2)
            )
        x_data_list = """"""
        y_data_list = """"""
        trace_num_10_list = """"""
        ...
        for jj in range(len(d1s)):
            for ii in range(0, self.size):
                x_data_list += """data.x{jj}_{trace_indx}.slice(trace_slice_begin,trace_slice_end), """.format(
                    jj=jj + 1, divid=self.divid, trace_indx=str(ii)
                )
                y_data_list += """data.y{jj}_{trace_indx}.slice(trace_slice_begin,trace_slice_end), """.format(
                    jj=jj + 1, divid=self.divid, trace_indx=str(ii)
                )
                trace_num_10_list += """{trace_num_10}, """.format(
                    trace_num_10=str(2 * jj * self.size + 2 * ii + 1 - 1)
                )
                trace_num_20_list += """{trace_num_20}, """.format(
                    trace_num_20=str(2 * jj * self.size + 2 * ii + 2 - 1)
                )
...
""".format(
            ...
            traces_cumul += """,trace{trace_num_1},trace{trace_num_2}""".format(
                trace_num_1=str(2 * self.size + 1), trace_num_2=str(2 * self.size + 2)
            )
            for ii in range(1, self.size):
                setup_trace2 += """
    let trace{trace_num_1}= {{
...
""".format(
            ...
            traces_cumul += """,trace{trace_num_1},trace{trace_num_2}""".format(
                trace_num_1=str(4 * self.size + 1), trace_num_2=str(4 * self.size + 2)
            )
            for ii in range(1, self.size):
                setup_trace3 += """
    let trace{trace_num_1}= {{
...
{close_json_code}}});
                ...
                config=config,
                layout=layout,
                load_jslibs_code=load_jslibs_code,
