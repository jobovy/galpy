...
def _fit_track_from_particles(
    ...
    ``cov_xyz`` (None for ``order=1``), ``smoothing_s``, and ``particles``
    (the raw ``xv`` array). ``smoothing_factor`` (>0) scales every
    spline's effective ``s`` after GCV / explicit-s choice.
...
class StreamTrack:
    ...
    * ``None``: pass-through; useful for any other custom curve parameter.
    ...
    Attributes
    ----------
    particles : ndarray of shape (6, N), or absent
        Present-day phase space ``(R, vR, vT, z, vz, phi)`` of the
        particles the track was fit to, in the layout that
    ...
    def from_particles(
        ...
        After the fit, ``inst.particles`` holds the raw ``xv`` array the
        fit saw and ``inst.smoothing_s`` holds the effective per-spline
    ...
    # -----------------------------------------------------------------
    # Custom sky frame (requires ``custom_sky_transform`` to be set)
    # -----------------------------------------------------------------
    @property
    ...
    def _require_custom(self):
        if self._custom_sky_transform is None:
            raise RuntimeError(
                "custom_sky_transform was not set at track construction; "
                ...
                "to the ``custom_sky_transform`` attribute on an existing "
    ...
    def phi1(self, tp, **kwargs):
        ...
        Requires ``custom_sky_transform`` to have been set at construction.
        ...
        p12 = coords.radec_to_custom(
            ra_dec[:, 0], ra_dec[:, 1], T=self._custom_sky_transform, degree=True
        )
    ...
    def cov(
        ...
        basis : str, optional
            ...
            - ``"customsky"``: ``(phi1, phi2, dist, pmphi1, pmphi2, vlos)``
              sky rotated by ``custom_sky_transform``. Requires
              ``custom_sky_transform`` to have been set.
        ro : float or Quantity, optional
    ...
    def plot(self, d1="x", d2="y", spread=0, **kwargs):
        ...
        d1, d2 : str
            ...
            phi1, phi2, pmphi1, pmphi2 if ``custom_sky_transform`` is set.
...
class StreamTrackPair:
    ...
    @property
    def particles(self):
        """Concatenated ``xv`` array from both arms in the leading-first
        ...
        """
        return numpy.hstack([self.leading.particles, self.trailing.particles])
    ...
    def custom_sky_transform(self):
        ...
        return self.leading.custom_sky_transform

    @custom_sky_transform.setter
    def custom_sky_transform(self, T):
        self.leading.custom_sky_transform = T
        self.trailing.custom_sky_transform = T
