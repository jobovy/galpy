...
def load_libgalpy():
    ...
    for path in sys.path:
        ...
        try:
            if sys.platform == "win32" and sys.version_info >= (
                ...
                _lib = ctypes.CDLL(
                    os.path.join(path, "libgalpy%s" % _ext_suffix), winmode=0x008
                )
            else:
                _lib = ctypes.CDLL(os.path.join(path, "libgalpy%s" % _ext_suffix))
        except OSError as e:
            if os.path.exists(
                os.path.join(path, "libgalpy%s" % _ext_suffix)
            ):  # pragma: no cover
                outerr = e
            _lib = None
        else:
    ...
    if _lib is None:  # pragma: no cover
        if not outerr is None:
            warnings.warn(
                "libgalpy C extension module not loaded, because of error '%s' "
                % outerr,
                galpyWarning,
            )
        else:
            warnings.warn(
                "libgalpy C extension module not loaded, because libgalpy%s image was not found"
                % _ext_suffix,
                galpyWarning,
            )
...
def load_libgalpy_actionAngleTorus():
    ...
    for path in sys.path:
        ...
        try:
            if sys.platform == "win32" and sys.version_info >= (
                ...
                # winmode=0x008 is easy-going way to call LoadLibraryExA
                _lib = ctypes.CDLL(
                    os.path.join(path, "libgalpy_actionAngleTorus%s" % _ext_suffix),
                    winmode=0x008,
                )
            else:
                _lib = ctypes.CDLL(
    ...
    if _lib is None:  # pragma: no cover
        if not outerr is None:
            warnings.warn(
                "libgalpy_actionAngleTorus C extension module not loaded, because of error '%s' "
                % outerr,
                galpyWarningVerbose,
            )
        else:
            warnings.warn(
                "libgalpy_actionAngleTorus C extension module not loaded, because libgalpy%s image was not found"
                % _ext_suffix,
                galpyWarningVerbose,
            )
