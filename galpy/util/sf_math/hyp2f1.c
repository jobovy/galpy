...
double a, b, c, x;
{
	...
	q *= pow(-x, -b);
	t1 = gsl_sf_gamma(c);
	s = t1 * gsl_sf_gamma(b - a) / (gsl_sf_gamma(b) * gsl_sf_gamma(c - a));
	y = t1 * gsl_sf_gamma(a - b) / (gsl_sf_gamma(a) * gsl_sf_gamma(c - b));
	return s * p + y * q;
    }
    else if (x < -1.0) {
	if (fabs(a) < fabs(b)) {
	...
	if (x > 0.0) {
	    ...
	    if (d <= 0.0)
		goto hypdiv;
	    y = gsl_sf_gamma(c) * gsl_sf_gamma(d) / (gsl_sf_gamma(p) * gsl_sf_gamma(r));
	    goto hypdon;
	}
	if (d <= -1.0)
	    goto hypdiv;
...
double *loss;
{
	...
	if (fabs(d - id) > EPS) {
	     ...
	     * for negative integer a or b, since the psi and gsl_sf_gamma functions
	     * involved have poles.
	     */
	    ...
	    /* sum for t = 0 */
	    y = gsl_sf_psi(1.0) + gsl_sf_psi(1.0 + e) - gsl_sf_psi(a + d1) - gsl_sf_psi(b + d1) - ax;
	    y /= gsl_sf_gamma(e + 1.0);

	    p = (a + d1) * (b + d1) * s / gsl_sf_gamma(e + 2.0);	/* Poch for t=1 */
	    t = 1.0;
	    do {
		r = gsl_sf_psi(1.0 + t) + gsl_sf_psi(1.0 + t + e) - gsl_sf_psi(a + t + d1)
		    - gsl_sf_psi(b + t + d1) - ax;
		q = p * r;
		y += q;
		p *= s * (a + t + d1) / (t + 1.0);
		p *= (b + t + d1) / (t + 1.0 + e);
	    ...
	    if (id == 0.0) {
		y *= gsl_sf_gamma(c) / (gsl_sf_gamma(a) * gsl_sf_gamma(b));
		goto psidon;
	    }
	  ...
	  nosum:
	    p = gsl_sf_gamma(c);
	    y1 *= gsl_sf_gamma(e) * p / (gsl_sf_gamma(a + d1) * gsl_sf_gamma(b + d1));

	    y *= p / (gsl_sf_gamma(a + d2) * gsl_sf_gamma(b + d2));
	    if ((aid & 1) != 0)
		y = -y;
