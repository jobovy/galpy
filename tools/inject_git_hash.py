...
def inject_git_hash_in_file(filepath, search_pattern, replace_pattern):
    ...
    def replacer(match):
        ...
        result = replace_pattern.format(version=version, git_hash=git_hash)
    ...
    new_content = re.sub(search_pattern, replacer, content, flags=re.MULTILINE)
...
def main():
    ...
    for file_info in files_to_update:
        if inject_git_hash_in_file(
            file_info["path"], file_info["search"], file_info["replace"]
        ):
            success_count += 1
